import { mysqlTable, mysqlSchema, AnyMySqlColumn, index, foreignKey, primaryKey, int, date, tinyint, mediumtext, longtext, tinytext } from "drizzle-orm/mysql-core"
...
export const action = mysqlTable("Action", {
	...
	grantId: int("Grant_id").notNull().references(() => grant.id),
	stateId: int("State_id").notNull().references(() => state.id),
},
(table) => {
	return {
		...
		actionIdPollIdGrantIdStateIdPk: primaryKey({ columns: [table.id, table.pollId, table.grantId, table.stateId], name: "Action_id_Poll_id_Grant_id_State_id_pk"}),
	}
});
...
(table) => {
	return {
		...
		answerIdQuestionIdPk: primaryKey({ columns: [table.id, table.questionId], name: "Answer_id_Question_id_pk"}),
	}
});

export const earnedMoney = mysqlTable("EarnedMoney", {
...
(table) => {
	return {
		...
		earnedMoneyIdUserIdPollIdPk: primaryKey({ columns: [table.id, table.userId, table.pollId], name: "EarnedMoney_id_User_id_Poll_id_pk"}),
	}
});
...
(table) => {
	return {
		fkGrantAnswer1Idx: index("fk_Grant_Answer1_idx").on(table.answerId, table.answerQuestionId),
		...
		grantIdRoleIdUserIdPk: primaryKey({ columns: [table.id, table.roleId, table.userId], name: "Grant_id_Role_id_User_id_pk"}),
	}
});
...
(table) => {
	return {
		fkQualificationSpecialty1Idx: index("fk_Qualification_Specialty1_idx").on(table.specialtyId),
		fkQualificationUser1Idx: index("fk_Qualification_User1_idx").on(table.userId),
		qualificationIdUserIdSpecialtyIdPk: primaryKey({ columns: [table.id, table.userId, table.specialtyId], name: "Qualification_id_User_id_Specialty_id_pk"}),
	}
});
...
(table) => {
	return {
		...
		questionIdPollIdPk: primaryKey({ columns: [table.id, table.pollId], name: "Question_id_Poll_id_pk"}),
	}
});
...
(table) => {
	return {
		specialtyIdPk: primaryKey({ columns: [table.id], name: "Specialty_id_pk"}),
	}
});
